...
from .grad_loss import grad_loss
from .L1_grad_loss import L1_grad_loss, L1_rgb_grad_loss


class SomGANModel(BaseModel):
    """
    This class implements the S2OMGAN model.
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(no_dropout=True)  # default SOMGAN did not use dropout
        if is_train:
    ...
    def __init__(self, opt):
        """Initialize the SOMGAN class.

        Parameters:
            opt (Option class)-- stores all the experiment flags; needs to be a subclass of BaseOptions
        """
        self.canny_loss = None
        ...
        if self.isTrain:
            ...
            self.criterionCycleB = self.l1_origin_plus_rgb_grad_plus_gradloss #torch.nn.L1Loss()
            self.criterionCycleAP= torch.nn.L1Loss()
            self.criterionCycleBP= self.l1_origin_plus_rgb_grad_plus_gradlossP #torch.nn.L1Loss()
    ...
    def backward_G(self, lambda_paired_loss = 0.):
        ...
        self.loss_cycle_B = self.criterionCycleB(self.rec_B, self.real_B) * lambda_B
        # combined loss and calculate gradients
                    
        # Paired loss
        if lambda_paired_loss > 0:
            self.loss_paired_AB = self.criterionCycleBP(self.fake_B, self.real_B) * lambda_B * lambda_paired_loss
            self.loss_G = self.loss_G_A + self.loss_idt_A + self.loss_paired_AB
            self.loss_G.backward()
            
            self.loss_paired_BA = self.criterionCycleAP(self.fake_A, self.real_A) * lambda_A * lambda_paired_loss * 10.
            self.loss_G = self.loss_G_B + self.loss_idt_B + self.loss_paired_BA
            self.loss_G.backward()
        else:
    ...
    def optimize_parameters(self, lambda_paired_loss = 0., epoch_ratio = 0.):
        ...
        self.backward_G(lambda_paired_loss) # calculate gradients for G_A and G_B
